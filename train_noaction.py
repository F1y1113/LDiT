...
def main(args):
    ...
    model = CDiT_models[config['model']](context_size=num_cond, input_size=latent_size, in_channels=4, use_instruction=config.get("use_instruction", False)).to(device)
    ...
    if os.path.isfile(latest_path) or config.get('from_checkpoint', 0):
        ...
        # 修正 map_location，确保为字符串类型
        map_location_str = f"cuda:{device}" if torch.cuda.is_available() else "cpu"
        latest_checkpoint = torch.load(latest_path, map_location=map_location_str, weights_only=False) 
    ...
    #     print("[DEBUG] Detected PEFT adapter config:")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for x, y, rel_t, instruction in loader:
            ...
            if train_steps % args.log_every == 0:
                ...
                # if rank == 0:
                #     vis_dir = os.path.join(experiment_dir, "train_vis_gt")
                #     os.makedirs(vis_dir, exist_ok=True)

                #     x_gt_pixels = x[:, num_cond:].flatten(0, 1).clone().detach().cpu()  # Ground-truth target frames
                #     x_gt_pixels = x_gt_pixels * 0.5 + 0.5  # Unnormalize [-1,1] → [0,1]

                #     for i in range(min(8, x_gt_pixels.shape[0])):
                #         save_path = os.path.join(vis_dir, f"{train_steps:07d}_gt_{i}.png")
                #         torchvision.utils.save_image(x_gt_pixels[i], save_path)
            ...
            if train_steps % args.eval_every == 0 and train_steps > 0:
                ...
                sim_score = evaluate(ema, tokenizer, diffusion_eval, test_dataset, rank, config["batch_size"], config["num_workers"], latent_size, device, save_dir, args.global_seed, bfloat_enable, num_cond, train_steps=train_steps)
                dist.barrier()
                eval_end_time = time()
...
@torch.no_grad()
def evaluate(model, vae, diffusion, test_dataloaders, rank, batch_size, num_workers, latent_size, device, save_dir, seed, bfloat_enable, num_cond, train_steps, eval_fraction=0.02):
    import lpips
    ...
    # Set up perceptual metrics
    lpips_loss_fn = lpips.LPIPS(net='alex').to(device)
    dreamsim_loss_fn, dreamsim_preprocess = dreamsim(pretrained=True, device=device)
    fid_metric = FrechetInceptionDistance(feature_dim=2048).to(device)

    save_imgs = False  # only save 10 images once

    for idx, (x, y, rel_t, instruction) in enumerate(loader):
        ...
        instruction = sum([[i] * (x.shape[1] - num_cond) for i in instruction], [])
        ...
        with torch.amp.autocast('cuda', enabled=True, dtype=torch.bfloat16):
            ...
            # Calculate basic dreamsim score
            res = eval_model(x_start_pixels, samples)
            score += res.sum()
            n_samples += len(res)

            # Calculate LPIPS, DreamSim, FID
            x_start_pixels_lpips = (x_start_pixels * 2) - 1
            samples_lpips = (samples * 2) - 1

            lpips_dist = lpips_loss_fn(x_start_pixels_lpips, samples_lpips).mean().item()

            dreamsim_x = torch.stack([dreamsim_preprocess(transforms.ToPILImage()(img.cpu().float())).squeeze(0) for img in x_start_pixels])
            dreamsim_y = torch.stack([dreamsim_preprocess(transforms.ToPILImage()(img.cpu().float())).squeeze(0) for img in samples])

            dreamsim_dist = dreamsim_loss_fn(dreamsim_x.to(device), dreamsim_y.to(device)).mean().item()

            fid_metric.update(x_start_pixels.float(), is_real=True)
            fid_metric.update(samples.float(), is_real=False)

            if rank == 0 and not save_imgs:
                os.makedirs(save_dir, exist_ok=True)
                for i in range(min(samples.shape[0], 10)):
                    fig, ax = plt.subplots(1, 3, figsize=(9, 3), dpi=256)
                    
                    ax[0].imshow(x_cond_pixels[i, -1].float().permute(1, 2, 0).cpu().numpy().clip(0, 1))
                    ax[0].set_title("Condition", fontsize=6)
                    ax[0].axis('off')
                    
                    ax[1].imshow(x_start_pixels[i].float().permute(1, 2, 0).cpu().numpy().clip(0, 1))
                    ax[1].set_title("Ground Truth", fontsize=6)
                    ax[1].axis('off')
                    
                    ax[2].imshow(samples[i].float().permute(1, 2, 0).cpu().numpy().clip(0, 1))
                    ax[2].set_title("Prediction", fontsize=6)
                    ax[2].axis('off')
                    
                    plt.tight_layout()
                    save_path = os.path.join(save_dir, f"{train_steps:07d}_triplet_{i}.png")
    ...
    if rank == 0:
        print(f"🔍 [Eval Metrics] LPIPS: {lpips_dist:.4f}, DreamSim: {dreamsim_dist:.4f}, FID: {fid_value:.2f}, Perceptual Loss (DreamSim Model): {sim_score.item():.4f}")

        os.makedirs(save_dir, exist_ok=True)
        metrics_to_save = {
            "lpips": float(lpips_dist),
            "dreamsim": float(dreamsim_dist),
            "fid": float(fid_value),
            "dreamsim_model_perceptual_loss": float(sim_score.item())
        }
        save_path = os.path.join(save_dir, f"eval_metrics_{train_steps:07d}.json")
