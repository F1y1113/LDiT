...
class InstructionEmbedder(nn.Module):
    def __init__(self, hidden_size, model_name='openai/clip-vit-base-patch16', freeze=True):
        super().__init__()
    ...
    def forward(self, instructions):
        """
        instructions: List[str] of batch instructions
        Returns: (B, hidden_size)
        """
        encoded = self.tokenizer(instructions, return_tensors='pt', padding=True, truncation=True).to(self.proj.weight.device)
        outputs = self.encoder(**encoded)
        cls_embedding = outputs.last_hidden_state[:, 0, :]  # Use [CLS] token from CLIP
...
class ActionEmbedder(nn.Module):
    """
    Embeds 3D position (x, y, z) and 3D rotation (yaw, pitch, roll) into a vector representation.
    """
    def __init__(self, hidden_size, frequency_embedding_size=256):
        super().__init__()
        # Divide hidden_size into 6 parts now
        hsize = hidden_size // 6
        ...
        self.yaw_emb = TimestepEmbedder(hsize, frequency_embedding_size)
        self.pitch_emb = TimestepEmbedder(hsize, frequency_embedding_size)
        self.roll_emb = TimestepEmbedder(hidden_size - 5 * hsize, frequency_embedding_size)  

    def forward(self, xyz_ypr):
        """
        xyz_ypr: tensor of shape [..., 6]
        """
        return torch.cat([
            self.x_emb(xyz_ypr[..., 0:1]),
            self.y_emb(xyz_ypr[..., 1:2]),
            self.z_emb(xyz_ypr[..., 2:3]),
            self.yaw_emb(xyz_ypr[..., 3:4]),
            self.pitch_emb(xyz_ypr[..., 4:5]),
            self.roll_emb(xyz_ypr[..., 5:6]),
        ], dim=-1)
...
class CDiT(nn.Module):
    ...
    def __init__(
        ...
        if self.use_instruction:
            self.instr_embedder = InstructionEmbedder(hidden_size)
    ...
    def initialize_weights(self):
        ...
        # # --- Initialize rotation embedders ---
        # nn.init.normal_(self.y_embedder.yaw_emb.mlp[0].weight, std=0.02)
        # nn.init.normal_(self.y_embedder.yaw_emb.mlp[2].weight, std=0.02)

        # nn.init.normal_(self.y_embedder.pitch_emb.mlp[0].weight, std=0.02)
        # nn.init.normal_(self.y_embedder.pitch_emb.mlp[2].weight, std=0.02)

        # nn.init.normal_(self.y_embedder.roll_emb.mlp[0].weight, std=0.02)
        # nn.init.normal_(self.y_embedder.roll_emb.mlp[2].weight, std=0.02)
    ...
    def forward(self, x, t, *, x_cond=None, rel_t=None, instruction=None, y=None):
        """
        Forward pass of DiT.
        ...
        c = t + time_emb
        if self.use_instruction and instruction is not None:
            instr_emb = self.instr_embedder(instruction)  # (B, hidden_size)
            c = c + instr_emb
