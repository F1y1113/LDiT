...
def model_forward_wrapper(all_models, curr_obs, curr_delta, num_timesteps, latent_size, device, num_cond, num_goals=1, rel_t=None, instruction=None, progress=False):
    ...
    with torch.amp.autocast('cuda', enabled=True, dtype=torch.bfloat16):
        ...
        # if instruction is not None and isinstance(instruction, list):
...
def generate_rollout(args, output_dir, rollout_fps, idxs, all_models, obs_image, gt_image, delta, num_cond, device, instruction=None):
    ...
    for i in range(gt_image.shape[1]):
        ...
        if args.gt:
            ...
            x_pred_pixels = model_forward_wrapper(all_models, curr_obs, curr_delta, rollout_stride, args.latent_size, num_cond=num_cond, num_goals=1, device=device, instruction=instruction)
...
def generate_time(args, output_dir, idxs, all_models, obs_image, gt_output, delta, secs, num_cond, device, instruction=None):
    delta = delta[..., :3]
    ...
    for sec, timestep in zip(secs, eval_timesteps):
        ...
        if args.gt:
            ...
            x_pred_pixels = model_forward_wrapper(all_models, obs_image, curr_delta, timestep, args.latent_size, num_cond=num_cond, num_goals=1, device=device, instruction=instruction)
...
def main(args):
    ...
    for dataset_name in dataset_names:
        ...
        for data_iter_step, (idxs, obs_image, gt_image, delta, instruction) in enumerate(metric_logger.log_every(curr_data_loader, print_freq, header)):
            with torch.amp.autocast('cuda', enabled=True, dtype=torch.bfloat16):
                ...
                if args.eval_type == 'rollout':
                    for rollout_fps in args.rollout_fps_values:
                        ...
                        generate_rollout(args, curr_rollout_output_dir, rollout_fps, idxs, model_lst, obs_image, gt_image, delta, num_cond, device, instruction=instruction)
                    ...
                    generate_time(args, curr_time_output_dir, idxs, model_lst, obs_image, gt_image, delta, secs, num_cond, device, instruction=instruction)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--exp", type=str, default='config/nwm_cdit_ha.yaml', help="experiment name")
    parser.add_argument("--ckp", type=str, default='0040000')
    parser.add_argument("--num_sec_eval", type=int, default=5)
    parser.add_argument("--input_fps", type=int, default=4)
    parser.add_argument("--datasets", type=str, default='ha_vln', help="dataset name")
    ...
    parser.add_argument("--gt", type=int, default=1, help="set to 1 to produce ground truth evaluation set")
